...
type Note = Record<{
    ...
    tags: string[] 
    archived?: boolean
    favorite?: boolean // Added the favorite field
}>
...
export function getNotes(): Result<Vec<Note>, string> {
    return Result.Ok(notesStorage.values().filter((note) => note.owner.toString() === ic.caller.toString()))
}
...
export function getNotesByTag(tag: string): Result<Vec<Note>, string> {
    return Result.Ok(notesStorage.values().filter((note) => note.owner.toString() === ic.caller.toString() && note.tags.includes(tag)))
}
...
export function addTagsToNote(id: string, tags: string[]): Result<Note, string> {
    return match(notesStorage.get(id), {
        ...
        None: () => Result.Err<Note, string>(`Couldn't add tags to note with id=${id}. Note not found`)
    })
}
...
export function addNote(payload: NotePayload): Result<Note, string> {
    ...
    const note: Note = { owner: ic.caller(), id: uuidv4(), created_at: ic.time(), updated_at: Opt.None, tags: [], ...payload }
...
export function deleteNote(id: string): Result<Note, string> {
    return match(notesStorage.get(id), {
        ...
        None: () => Result.Err<Note, string>(`Couldn't delete a note with id=${id}. Note not found.`)
    })
}
...
export function searchNotes(query: string): Result<Vec<Note>, string> {
    ...
    const filteredNotes = notesStorage.values().filter((note) =>
...
export function unarchiveNote(id: string): Result<Note, string> {
    return match(notesStorage.get(id), {
        ...
        None: () => Result.Err<Note, string>(`Couldn't unarchive note with id=${id}. Note not found`)
    })
}
...
export function favoriteNote(id: string): Result<Note, string> {
    return match(notesStorage.get(id), {
        ...
        None: () => Result.Err<Note, string>(`Couldn't favorite note with id=${id}. Note not found`)
    })
}
...
export function unfavoriteNote(id: string): Result<Note, string> {
    return match(notesStorage.get(id), {
        ...
        None: () => Result.Err<Note, string>(`Couldn't unfavorite note with id=${id}. Note not found`)
    })
}
